...
helper.signed_tr_json = function (tr, private_keys) {
    ...
    tr.signatures = (function () {
        var result = [];
        for (var i = 0; 0 < private_keys.length
            ? i < private_keys.length
            : i > private_keys.length; 0 < private_keys.length ? i++ : i++) {
 ...
 */
helper.template = function (serializer_operation_type_name, debug) {
    if (debug === void 0) { debug = { use_default: true, annotate: true }; }
...
export var TransactionHelper = helper;
