...
var key = {
    ...
    aes_checksum: function (password) {
        ...
        return { aes_private: Aes.fromSeed(secret), checksum: checksum_string };
    },
    /**
    ...
    aes_private: function (password, key_checksum) {
        var _a = key_checksum.split(","), iterations = _a[0], salt = _a[1], checksum = _a[2];
        var secret = salt + password;
    ...
    random32ByteBuffer: function (entropy) {
        if (entropy === void 0) { entropy = this.browserEntropy(); }
    ...
    suggest_brain_key: function (dictionary, entropy) {
    ...
    get_brainPrivateKey: function (brainKey, sequence) {
    ...
    addresses: function (pubkey, address_prefix) {
        ...
        // S L O W
        var address_string = [
            Address.fromPublic(public_key, false, 0).toString(address_prefix),
            Address.fromPublic(public_key, true, 0).toString(address_prefix),
            Address.fromPublic(public_key, false, 56).toString(address_prefix),
            Address.fromPublic(public_key, true, 56).toString(address_prefix),
