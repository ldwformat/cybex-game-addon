...
var Aes = /** @class */ (function () {
    ...
    Aes.fromSha512 = function (hash) {
        assert.strictEqual(hash.length, 128, "A Sha512 in HEX should be 128 characters long, instead got " + hash.length);
        var iv = encHex.parse(hash.substring(64, 96));
        var key = encHex.parse(hash.substring(0, 64));
        return new Aes(iv, key);
    };
    Aes.fromBuffer = function (buf) {
        assert(Buffer.isBuffer(buf), "Expecting Buffer");
        assert.strictEqual(buf.length, 64, "A Sha512 Buffer should be 64 characters long, instead got " + buf.length);
     ...
     *   @return {Buffer}
     */
    Aes.decrypt_with_checksum = function (private_key, public_key, nonce, message, legacy) {
     ...
     */
    Aes.prototype.decryptHexToText = function (cipher, encoding) {
